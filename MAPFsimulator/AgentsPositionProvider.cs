...
namespace MAPFsimulator
{
    public static class AgentsPositionProvider
    {
        ...
        public static AgentState GetAgentState(int agentId)
        {
            if (_agentStates.TryGetValue(agentId, out var state))
            {
                return state;
            }
            throw new AgentIdNotFoundException(agentId);
        }
        
        public static void UpdateAgentPosition(int agentId, Vertex current, Vertex next, Dictionary<Vertex, List<int>> planToTarget)
        {
